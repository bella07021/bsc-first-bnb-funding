...
到账时间分组按具体 CEX 热钱包地址分别计算，同一交易所的不同热钱包不会合并。组内按该热钱包与各接收地址的首次到账时间升序排列，相邻间隔不超过 20 分钟且连续至少 2 个地址。一个接收地址收到过多个 CEX 热钱包的 BNB 时，会分别显示多条关系，并使用 `1-1`、`1-2` 形式的父子序号。页面与 Excel 导出会同时显示组内地址数和对应序号。
